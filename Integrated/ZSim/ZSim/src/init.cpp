...
#include "weave_bw_lat_mem_ctrl.h" // bandwidth--latency curve memory simulator
#include "zsim.h"
#include "acm.h"
#include <iostream>
...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    } else if (replType == "MN4LLC") {
        rp = new MN4LLCReplPolicy(numLines);
    } else if (replType == "MN4L2") {
        rp = new MN4L2ReplPolicy(numLines);
...
DDRMemory* BuildDDRMemory(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string name, const string& prefix) {
    uint32_t ranksPerChannel = config.get<uint32_t>(prefix + "ranksPerChannel", 2);
    uint32_t banksPerRank = config.get<uint32_t>(prefix + "banksPerRank", 16);  // DDR3 std is 8
...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
    ...
    if (type == "Simple") {
        ...
        string application = "bwLatCurves";//config.get<const char*>("sim.stats");
...
static void InitSystem(Config& config) {
    ...
    if (!zinfo->traceDriven) {
        ...
        // we need to know memory type because we want to warranty QS of mem
        ...
        for (const char* group : coreGroupNames) {
            ...
            if (type != "Null") {
                ...
                for (uint32_t j = 0; j < cores; j++) {
                    ...
                    acmInfo.numberOfACMs = config.get<uint32_t>("sys.mem.acm.numberOfACMs", 1);
                    ...
                    if (type == "Simple") {
                        ...
                        // add the quality of service feature 
                        if(memType == "bw-lat-cruves") { 
                            ocore->setQsEnable();
                            ocore->setMainMemory(mems);
                        }
...
// If the simulation gets executed in the same path as in the previous run
// all the h5 files are getting "planked" on the new execution. 
...
static void InitGlobalStats() {
    ...
    ProxyStat* reqCyclesStat = new ProxyStat();
    reqCyclesStat->init("reqCycles", "Requeue Cycles", &zinfo->requeueCycles);
    zinfo->rootStat->append(reqCyclesStat);
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
    ...
    // // Adding a hack for any PIM wait cycles that left in memory simulator.
    // printf("zsim finished in init.cpp\n");
    // // cores[0] type is core*
    // Core* mycore = zinfo->cores[0];
    // // get the type of the cores 
    // string prefix = string("sys.cores.") + string("skylake") + ".";
    // string coreType = config.get<const char*>(prefix + "type", "Simple");
    // // cout << coreType << endl;
    // // printf("%s \n", coreType);
    // cout << coreType << endl;
    // if(coreType=="OOO")
    // {
    //     cout << "init.cpp: " << endl;
    //     OOOCore* myoooCore = (OOOCore*) mycore;
    //     uint64_t remaining_cycles = myoooCore->wait_for_PIM_cycles();
    //     cout << "remaining_cycles: " << remaining_cycles << endl;
    // }
