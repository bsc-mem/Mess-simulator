...
def plot_limit_line(ax, mem_cfg):
    ...
    ax.text(x=max_bw, y=550, s=f'Max. theoretical BW = {round(max_bw)} GB/s', horizontalalignment='right')
...
def print_application_data_points(ax, data_points_baseline, curves, config, color):
    ...
    ax.text(x=x_points_baseline[0]-12, y=y_points_baseline[1]+20 , s='Copy \nScale', horizontalalignment='left', fontsize=28)
...
def final_curves(config, filepath, dfs_rw, bw_column='bandwidth_smooth', lat_column='latency_smooth', lat_unit='cycles'):
    ...
    # stream_data = []
    # stream_data.append((66,101.46)) # copy
    # stream_data.append((66,97.95)) # scale
    # stream_data.append((75,103.50)) # add
    # stream_data.append((75,101.77)) # triad
