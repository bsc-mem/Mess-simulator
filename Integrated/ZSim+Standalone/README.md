...
**Note:** This repository is an updated version of the original ZSim directory.  
We maintain this directory in sync with updates to the standalone ZSim version.

### Changes from the Original Integration

Compared to the original ZSim integration, we renamed the `bw_lat` memory simulator to `Mess`.  
The integration of the **Mess** simulator includes the following changes:

- **New source files added:**
  - `weave_mess_mem_ctrl.h`
  - `mess_mem_ctrl.cpp`
  - `mess_mem_ctrl.h`

- **Modified files:**
  - `init.cpp` is updated to instantiate the Mess memory model.

- **New test configuration:**
  - `tests/mess.sb` provides an example configuration file for using Mess as the memory simulator for ZSim.
