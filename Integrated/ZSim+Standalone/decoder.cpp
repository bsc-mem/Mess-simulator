...
void Decoder::emitDiv(Instr& instr, DynUopVec& uops) {
    ...
    if (srcs == 3 && dsts == 3) {
        ...
        emitBasicOp(instr, uops, lat, PORTS_015);
    } else {
  ...
  //   }
    ...
    switch (category) {
        ...
        case /*XC(X)87_ALU*/ XC(X87_ALU):
			switch (opcode) {
                case XO(FADDP):
                    emitBasicOp(instr, uops, 289, PORT_5);  // huge. based on result from ubench
                    break;

                case XO(FSUBP): // wrong 
                case XO(FSUBR): 
                    emitBasicOp(instr, uops, 261, PORT_6); //3, page 630 intel appendix c
                ...
                case XO(FDIVP):
                case XO(FDIVRP):
                    emitBasicOp(instr, uops, 292, PORT_0,61); //
                    break;
				...
				case XO(FNINIT): //David: Ports 0,1,5,
					// emitBasicOp(instr, uops, 74, PORTS_015, 73);
                    {
        ...
        case XC(SSE): // sse is 61
            {
                ...
                switch (opcode) {
                    ...
                    case XO(DIVPS):
                    case XO(DIVSS):
                        lat = 11; //Pouya, from mubench //rsv agner
                    ...
                    case XO(DIVPD):
                    case XO(DIVSD):
                        lat = 13; //Pouya, from mubench //rsv agner
                        ports = PORT_0; //rsv   //non-pipelined
                        //extraSlots = lat - 12;
                        break;
     ...
     //                }
        ...
        case XC(CONVERT): //part of SSE //Pouya: modified some function to contain extra slots.
            switch (opcode) {
                ...
                case XO(CDQE): //David:Matches doc
                    emitBasicOp(instr, uops, 1, PORTS_015 | PORT_6);
                    break;
                case XO(CWD): //Pouya: Two uops, latency goes down if we do so
					{
                    uint32_t lats[] = {2, 2};
        ...
        case XC(AES): //All David
			switch (opcode) {
				...
				case XO(AESENCLAST):
					...
					emitBasicOp(instr, uops, 5, PORT_0, 4); // at least for now 
                    break;
        ...
        case XC(MMX): //Pouya:40  //David: This should not be very important
			switch (opcode) {
                case XO(PSHUFW):  //
                    emitBasicOp(instr, uops, 1, PORT_5);
                    break;
        ...
        case XC(MISC):
			switch (opcode) {
				...
				case XO(PAUSE):
					...
					emitExecUop(0, 0, 0, 0, uops, 11, PORT_0, 10); //9, longest first //David, changed from 9, 8
					emitExecUop(0, 0, 0, 0, uops, 3, PORT_1); //NOTE: latency does not matter //David, why?
					emitExecUop(0, 0, 0, 0, uops, 1, PORT_5);
					emitExecUop(0, 0, 0, 0, uops, 1, PORT_5);
					emitExecUop(0, 0, 0, 0, uops, 1, PORT_5);
					emitExecUop(0, 0, 0, 0, uops, 1, PORT_5);
					emitExecUop(0, 0, 0, 0, uops, 1, PORT_5);
